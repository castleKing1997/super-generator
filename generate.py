...
def change_package(filename):
    text = np.loadtxt(filename, dtype="str", encoding="utf-8", delimiter="xxxxxxx")
    text = pd.Series(text)
...
for type in ("main", "test"):
    ...
    for i in range(len(sub_dirs)):
        out_dir.joinpath(name, "src", type, "java", *sub_dirs[:i], sub_dirs[i]).mkdir(exist_ok=True)
    out_dir.joinpath(name, "src", type, "java", *sub_dirs, "tiny").mkdir(exist_ok=True)
# generate model and mapper
# pom.xml
pom_xml = np.loadtxt(str(template_dir.joinpath("pom.xml")), dtype="str", encoding="utf-8", delimiter="xxxxxxx")
...
np.savetxt(str(out_dir.joinpath(name, "pom.xml")), pom_xml.values, fmt="%s", encoding="utf-8")
...
for filename in ["generator.properties", "application.yml"]:
    property = np.loadtxt(str(template_dir.joinpath("src", "main", "resources", filename)), dtype="str", encoding="utf-8", delimiter="xxxxxxx")
    ...
    np.savetxt(str(out_dir.joinpath(name,  "src", "main", "resources", filename)), property.values, fmt="%s", encoding="utf-8")
# generatorConfig.xml
generator_config = np.loadtxt(str(template_dir.joinpath("src", "main", "resources", "generatorConfig.xml")), dtype="str", encoding="utf-8", delimiter="xxxxxxx")
...
np.savetxt(str(out_dir.joinpath(name,  "src", "main", "resources", "generatorConfig.xml")), generator_config.split("\n"), fmt="%s", encoding="utf-8")
# mybatis generator
out_dir.joinpath(name, *JAVA_MAIN, *sub_dirs, "tiny", "mbg").mkdir(exist_ok=True)
shutil.copy(str(template_dir.joinpath(*JAVA_MAIN, *temp_sub_dirs, "tiny", "mbg", "CommentGenerator.java")), str(out_dir.joinpath(name, *JAVA_MAIN, *sub_dirs, "tiny", "mbg")))
shutil.copy(str(template_dir.joinpath(*JAVA_MAIN, *temp_sub_dirs, "tiny", "mbg", "Generator.java")), str(out_dir.joinpath(name, *JAVA_MAIN, *sub_dirs, "tiny", "mbg")))
change_package(str(out_dir.joinpath(name, *JAVA_MAIN, *sub_dirs, "tiny", "mbg", "CommentGenerator.java")))
change_package(str(out_dir.joinpath(name, *JAVA_MAIN, *sub_dirs, "tiny", "mbg", "Generator.java")))
...
rm(str(out_dir.joinpath(name, *JAVA_MAIN, *sub_dirs, "tiny", "mbg", "CommentGenerator.java")))
rm(str(out_dir.joinpath(name, *JAVA_MAIN, *sub_dirs, "tiny", "mbg", "Generator.java")))
# generate myservice and mycontroller
out_dir.joinpath(name, *JAVA_MAIN, *sub_dirs, "tiny", "service").mkdir(exist_ok=True)
out_dir.joinpath(name, *JAVA_MAIN, *sub_dirs, "tiny", "service", "impl").mkdir(exist_ok=True)
out_dir.joinpath(name, *JAVA_MAIN, *sub_dirs, "tiny", "controller").mkdir(exist_ok=True)
for i, table_name in enumerate(mysql["tableNames"]):
    ...
    for tempfile, newfile in zip((["service", "PmsBrandService.java"], ["service", "impl", "PmsBrandServiceImpl.java"], ["controller", "PmsBrandController.java"]), \
        ...
        jfile = np.loadtxt(str(template_dir.joinpath(*JAVA_MAIN, *temp_sub_dirs, "tiny", *tempfile)), dtype="str", encoding="utf-8", delimiter="xxxxxxx")
        ...
        jfile = jfile.str.replace("Long id", "%s id"%mysql["idTypes"][i], regex=False)
        np.savetxt(str(out_dir.joinpath(name, *JAVA_MAIN, *sub_dirs, "tiny", *newfile)), jfile.values, fmt="%s", encoding="utf-8")
# other files
for package in ["common", "config"]:
    out_dir.joinpath(name, *JAVA_MAIN, *sub_dirs, "tiny", package).mkdir(exist_ok=True)
    proot = template_dir.joinpath(*JAVA_MAIN, *temp_sub_dirs, "tiny", package)
    for root, dirs, files in osw(str(proot)):
        root = Path(root)
        for dir in dirs:
            out_dir.joinpath(name, *JAVA_MAIN, *sub_dirs, "tiny", package, root.relative_to(proot), dir).mkdir(exist_ok=True)
        for file in files:
            newfile = out_dir.joinpath(name, *JAVA_MAIN, *sub_dirs, "tiny", package, root.relative_to(proot), file)
